...
export default function MotoBoy_1({ navigation }) {
  ...
  const pesquisarPorRua = async (pesquisa) => {
    const resultados = [];
    const querySnapshot = await firebase.firestore().collection("QUIDO").get();
  ...
  const apagarPedido = () => {
    ...
    const pedidoRef = firebase.firestore().collection("QUIDO").doc(selectedPedido);
  ...
  const baseCollectionNames = ["QUIDO"];
  ...
  const apagarTudo = async () => {
    const collectionRef = firebase.firestore().collection("QUIDO");
