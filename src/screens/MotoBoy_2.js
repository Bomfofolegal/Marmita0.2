...
export default function MotoBoy_1({ navigation }) {
  ...
  const pesquisarPorRua = async (pesquisa) => {
    const resultados = [];
    const querySnapshot = await firebase.firestore().collection("MIOJO").get();
  ...
  const apagarPedido = () => {
    ...
    const pedidoRef = firebase.firestore().collection("MIOJO").doc(selectedPedido);
  ...
  const baseCollectionNames = ["MIOJO"];
  ...
  const apagarTudo = async () => {
    const collectionRef = firebase.firestore().collection("MIOJO");
